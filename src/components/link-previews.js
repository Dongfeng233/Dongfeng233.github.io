...
export default function LinkPreviews() {
  ...
  const close = (restoreFocus = false) => { const trigger = active.current; trigger?.setAttribute("aria-expanded", "false"); active.current = null; activeSource.current = null; setPreview(null); if (restoreFocus) trigger?.focus({ preventScroll: true }); };
  const timer = useRef(null), closeTimer = useRef(null), popup = useRef(null), active = useRef(null), activeSource = useRef(null);
  useEffect(() => {
    ...
    loadIndex().then((items) => {
      ...
      for (const link of links) {
        ...
        const show = () => { clearTimeout(timer.current); clearTimeout(closeTimer.current); active.current?.setAttribute("aria-expanded", "false"); active.current = trigger; activeSource.current = link; trigger.setAttribute("aria-expanded", "true"); const rect = link.getBoundingClientRect(); setPosition({ left: Math.max(12, Math.min(rect.left, innerWidth - 352)), top: Math.max(70, Math.min(rect.bottom + 10, innerHeight - 340)) }); setPreview({ ...item, href: link.href }); };
        const enter = () => { clearTimeout(closeTimer.current); timer.current = setTimeout(show, 220); };
        const leave = () => { clearTimeout(timer.current); closeTimer.current = setTimeout(() => { if (active.current === trigger && !popup.current?.contains(document.activeElement) && document.activeElement !== link && document.activeElement !== trigger) close(); }, 180); };
    ...
    const dismiss = (event) => { if (!active.current || event.defaultPrevented) return; if (event.type === "keydown" && event.key !== "Escape") return; if (event.type === "pointerdown" && (popup.current?.contains(event.target) || event.target.closest(".link-peek-trigger"))) return; if (event.type === "keydown") { event.preventDefault(); event.stopImmediatePropagation(); } close(event.type === "keydown"); };
    const focus = (event) => { if (active.current && event.target !== active.current && event.target !== activeSource.current && !popup.current?.contains(event.target)) close(); };
    document.addEventListener("focusin", focus);
    document.addEventListener("pointerdown", dismiss); document.addEventListener("keydown", dismiss);
    return () => { alive = false; clearTimeout(timer.current); clearTimeout(closeTimer.current); cleanups.forEach((cleanup) => cleanup()); document.removeEventListener("focusin", focus); document.removeEventListener("pointerdown", dismiss); document.removeEventListener("keydown", dismiss); };
  }, []);
  if (!preview) return null;
  return createPortal(<aside id="internal-link-preview" tabIndex={-1} ref={popup} className="link-peek-card" role="dialog" aria-label={`预览 ${preview.title}`} style={position} onPointerEnter={() => clearTimeout(closeTimer.current)} onPointerLeave={() => { closeTimer.current = setTimeout(() => { if (!popup.current?.contains(document.activeElement)) close(); }, 180); }}><button className="link-peek-close" aria-label="关闭预览" onClick={() => close(true)}>×</button>{preview.image ? <img src={preview.image} alt="" loading="lazy" /> : null}<small>{preview.kind === "collection" ? "收藏的作品" : "站内阅读"}</small><h3>{preview.title}</h3><p>{preview.description || "打开查看完整内容。"}</p><a href={preview.href}>{preview.kind === "collection" ? "查看作品" : "阅读全文"} →</a></aside>, document.body);
}
