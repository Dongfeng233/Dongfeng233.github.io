...
页面内容由 `data/content/pages/zhouyi.md` 管理。在管理端“页面 → 读易”中可编辑标题、摘要、正文、地址与发布状态；设置中的“页面类型”选择“读易工作台”，可调整标题上方短句。正文显示在工作台上方。新建页面也可选择此类型。

页面导航由管理端“导航”维护；重命名页面时同步更新导航引用。草稿随发布流程保留在本机，公开页面、导航与站点地图按已发布内容生成。
