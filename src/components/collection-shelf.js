...
export default function CollectionShelf({ items, nodes }) {
  ...
  useEffect(() => { const open = () => { const id = new URLSearchParams(location.search).get("item"); if (id && items.some((item) => item.id === id)) { setExpanded(id); setType(""); setStatus(""); requestAnimationFrame(() => document.getElementById(`collection-${id}`)?.scrollIntoView({ block: "center" })); } }; open(); window.addEventListener("popstate", open); return () => window.removeEventListener("popstate", open); }, [items]);
