...
const ScrollTopAndComment = ({ showComments = true }) => {
  ...
  return (
    ...
    >
      {showComments ? (
        <button aria-label="跳转到评论" type="button" onClick={handleScrollToComment} className={btn}>
      ...
      <button aria-label="返回顶部" type="button" onClick={handleScrollTop} className={btn}>
        <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path
            fillRule="evenodd"
